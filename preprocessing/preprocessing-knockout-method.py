...
def extract_mean_features(source_path, target_path, dataset_name='Games', model_name='all-MiniLM-L6-v2'):
    """Extract mean text and image features (all items get same average features)"""
    # Load metadata from target path
    meta_file = os.path.join(target_path, f'meta-{dataset_name}.csv')
    df = pd.read_csv(meta_file)
    df.sort_values(by=['itemID'], inplace=True)
    ...
    # Encode and compute mean
    model = SentenceTransformer(model_name)
    sentence_embeddings = model.encode(sentences)
    avg_text_vector = np.mean(sentence_embeddings, axis=0)
    
    # Use mean vector for all items
    sentence_embeddings = np.tile(avg_text_vector, (len(sentences), 1))
    np.save(os.path.join(target_path, 'text_feat.npy'), sentence_embeddings)
    print('Mean text features saved!')
    
    # Process image features similarly
    def readImageFeatures(path):
        f = open(path, 'rb')
        while True:
            asin = f.read(10).decode('UTF-8')
    ...
    avg_img_vector = np.array(avg).mean(0).tolist()
    
    # Use mean vector for all items
    ret = [avg_img_vector for _ in range(len(item2id))]
    non_no = list(range(len(item2id)))
    
    np.save(os.path.join(target_path, 'image_feat.npy'), np.array(ret))
    np.savetxt(os.path.join(target_path, "missed_img_itemIDs.csv"), non_no, delimiter=",", fmt='%d')
    print('Mean image features saved!')
    
    return sentence_embeddings, np.array(ret)
...
def extract_constant_features(target_path, dataset_name='Games', text_dim=384, image_dim=4096):
    """Extract constant features (all ones) for text and image features"""
    # Load metadata to get number of items
    meta_file = os.path.join(target_path, f'meta-{dataset_name}.csv')
    df = pd.read_csv(meta_file)
    num_items = len(df)

    print(f'Generating constant features for {num_items} items')

    # Create constant embeddings
    text_embeddings = np.full((num_items, text_dim), 1, dtype=np.float32)
    image_embeddings = np.full((num_items, image_dim), 1, dtype=np.float32)

    # Save features
    np.save(os.path.join(target_path, 'text_feat.npy'), text_embeddings)
    np.save(os.path.join(target_path, 'image_feat.npy'), image_embeddings)
    
    # Create missed items file (all items are "missed" since we use zeros)
    non_no = list(range(num_items))
    np.savetxt(os.path.join(target_path, "missed_img_itemIDs.csv"), non_no, delimiter=",", fmt='%d')
    
    print('Zero text and image features saved!')
    return text_embeddings, image_embeddings


def extract_noise_features(target_path, dataset_name='Games', text_dim=384, image_dim=4096, 
                          random_seed=42, noise_std=0.1):
    """Extract random noise features for text and image features"""
    ...
    print(f'Generating noise features for {num_items} items')
    
    # Create random noise embeddings
    text_embeddings = np.random.normal(1, noise_std, (num_items, text_dim))
    image_embeddings = np.random.normal(1, noise_std, (num_items, image_dim))
    
    # Save features
    np.save(os.path.join(target_path, 'text_feat.npy'), text_embeddings)
    np.save(os.path.join(target_path, 'image_feat.npy'), image_embeddings)
    
    # Create missed items file (all items are "missed" since we use noise)
    non_no = list(range(num_items))
    np.savetxt(os.path.join(target_path, "missed_img_itemIDs.csv"), non_no, delimiter=",", fmt='%d')
    
    print('Noise text and image features saved!')
    return text_embeddings, image_embeddings


def extract_shuffle_features(source_path, target_path, dataset_name='Games', model_name='all-MiniLM-L6-v2', random_seed=42):
    """Extract text and image features, then shuffle their internal order randomly"""
    import numpy as np
    ...
    print(f'Extracting and shuffling features for dataset: {dataset_name}')
    
    # First extract normal text features
    meta_file = os.path.join(target_path, f'meta-{dataset_name}.csv')
    df = pd.read_csv(meta_file)
    df.sort_values(by=['itemID'], inplace=True)
    
    print(f'Processing text features for {df.shape[0]} items')
    ...
    # Encode sentences (original)
    model = SentenceTransformer(model_name)
    original_text_embeddings = model.encode(sentences)
    
    # Shuffle the text embeddings order
    num_items = original_text_embeddings.shape[0]
    text_shuffle_indices = np.random.permutation(num_items)
    shuffled_text_embeddings = original_text_embeddings[text_shuffle_indices]
    
    # Calculate text similarity
    text_similarity = cosine_similarity(
        original_text_embeddings.reshape(1, -1), 
        shuffled_text_embeddings.reshape(1, -1)
    )[0, 0]
    
    print(f'Text embeddings shuffled! Similarity between original and shuffled: {text_similarity:.6f}')
    print(f'Text shuffle mapping (first 10): {text_shuffle_indices[:10]}')
    
    # Save shuffled text features
    np.save(os.path.join(target_path, 'text_feat.npy'), shuffled_text_embeddings)
    
    # Now process image features
    def readImageFeatures(path):
        f = open(path, 'rb')
        while True:
            asin = f.read(10).decode('UTF-8')
    ...
    avg_vector = np.array(avg).mean(0).tolist()
    ...
    for i in range(len(item2id)):
        if i in feats:
            original_image_features.append(feats[i])
        else:
            non_no.append(i)
            original_image_features.append(avg_vector)
    
    original_image_features = np.array(original_image_features)
    
    # Shuffle image features with DIFFERENT indices than text
    image_shuffle_indices = np.random.permutation(num_items)
    shuffled_image_features = original_image_features[image_shuffle_indices]
    
    # Calculate image similarity
    image_similarity = cosine_similarity(
        original_image_features.reshape(1, -1), 
        shuffled_image_features.reshape(1, -1)
    )[0, 0]
    
    print(f'Image embeddings shuffled! Similarity between original and shuffled: {image_similarity:.6f}')
    print(f'Image shuffle mapping (first 10): {image_shuffle_indices[:10]}')
    print(f'Items missing image features: {len(non_no)}')
    
    # Calculate cross-modal correspondence after shuffling
    text_image_correspondence = np.sum(text_shuffle_indices == image_shuffle_indices) / num_items
    print(f'Cross-modal correspondence after shuffling: {text_image_correspondence:.6f} ({text_image_correspondence*100:.2f}%)')
    
    # Save shuffled image features
    np.save(os.path.join(target_path, 'image_feat.npy'), shuffled_image_features)
    np.savetxt(os.path.join(target_path, "missed_img_itemIDs.csv"), non_no, delimiter=",", fmt='%d')
    
    print('Shuffled text and image features saved!')
    
    return shuffled_text_embeddings, shuffled_image_features
...
def main():
    ...
    parser.add_argument('--method', type=str, default='default',
                       choices=['default', 'mean', 'zero', 'noise', 'one', 'constant-one', 'shuffle'],
                       help='Method for feature extraction: default (normal), mean (average), zero (all zeros), noise (random), one (all ones)')

    args = parser.parse_args()
    dataset_name = args.dataset
    method = args.method if args.method != 'default' else None
    ...
    if args.step in ['all', 'text', 'image']:
        if args.method == 'mean':
            print('\n=== Feature Extraction: Mean features ===')
            text_features, image_features = extract_mean_features(source_path, target_path, dataset_name)
        elif args.method == 'constant-one':
            print('\n=== Feature Extraction: Constant one features ===')
            text_features, image_features = extract_constant_features(target_path, dataset_name)
        elif args.method == 'noise':
            print('\n=== Feature Extraction: Noise features ===')
            text_features, image_features = extract_noise_features(target_path, dataset_name)
        elif args.method == 'shuffle':
            print('\n=== Feature Extraction: Shuffle features ===')
            text_features, image_features = extract_shuffle_features(source_path, target_path, dataset_name)
        else:  # default method
