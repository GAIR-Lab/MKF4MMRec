...
def extract_image_features(source_path, target_path, dataset_name='Games', batch_size=8):
    ...
    image_dir = os.path.join('../dataset', dataset_name, 'image')
    
    # 定义描述文件的路径
    text_output_filename = os.path.join(target_path, f'{dataset_name}_llava_descriptions.json')

    generated_texts = {}  # 用于内部处理，key: item_id, value: text

    # 检查描述文件是否存在
    if os.path.exists(text_output_filename):
        print(f"Found existing descriptions file: {text_output_filename}")
        print("Loading descriptions from file, skipping LLaVA generation.")
        with open(text_output_filename, 'r', encoding='utf-8') as f:
            generated_texts_for_json = json.load(f)
        
        # 将加载的数据转换为以 item_id 为 key 的字典
        for filename, text in generated_texts_for_json.items():
            asin = filename.replace('.jpg', '')
            if asin in item2id:
                item_id = item2id[asin]
                generated_texts[item_id] = text
        print(f"Loaded {len(generated_texts)} descriptions.")

    else:
        print("Descriptions file not found. Proceeding with LLaVA generation.")
        # --- 2. 加载所需模型 ---
        ...
        try:
            llava_tokenizer, llava_model, llava_image_processor, _ = load_pretrained_model(
                model_path=llava_model_path, model_base=None, model_name=get_model_name_from_path(llava_model_path),
                load_8bit=True, device=device, use_flash_attention_2=True
            )
            print("LLaVA loaded with Flash Attention 2.")
        except (ImportError, RuntimeError):
            print("Flash Attention 2 not available. Loading LLaVA without it.")
            llava_tokenizer, llava_model, llava_image_processor, _ = load_pretrained_model(
                model_path=llava_model_path, model_base=None, model_name=get_model_name_from_path(llava_model_path),
                load_8bit=True, device=device
            )
        llava_model.eval()
        print(f"LLaVA model loaded on {device}")

        # --- 4. 准备处理列表和 LLaVA Prompt ---
        prompt = "Describe this product in fluent English, Mention its category, colour, shape and any distinctive features."
        
        # 筛选出所有存在对应图片文件的有效 item
        items_to_process = []
        for item_id, asin in id2asin.items():
            image_path = os.path.join(image_dir, f"{asin}.jpg")
            if os.path.exists(image_path):
                items_to_process.append({'item_id': item_id, 'image_path': image_path})

        # --- 5. 阶段一：用 LLaVA 批量生成所有文本描述 ---
        print("Stage 1: Batch-generating text descriptions with LLaVA...")
        generated_texts_for_json = {} # 用于保存JSON，key: filename, value: text
        
        for i in tqdm(range(0, len(items_to_process), batch_size), desc="LLaVA Batch Processing"):
            batch_data = items_to_process[i:i + batch_size]
            batch_paths = [item['image_path'] for item in batch_data]
            batch_item_ids = [item['item_id'] for item in batch_data]

            try:
                # 1. 批量加载和处理图片
                images = [Image.open(p).convert('RGB') for p in batch_paths]
                image_tensor = process_images(images, llava_image_processor, llava_model.config).to(device, dtype=torch.float16)

                # 2. 批量构建 Prompt
                conv = conv_templates["llava_v1"].copy()
                roles = conv.roles
                inp = f"{roles[0]}: {DEFAULT_IMAGE_TOKEN}\n{prompt}"
                conv.append_message(roles[0], inp)
                conv.append_message(roles[1], None)
                prompt_full = conv.get_prompt()
                
                # 为批次中的每个图像复制 prompt
                input_ids = tokenizer_image_token(prompt_full, llava_tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(device)
                input_ids = input_ids.repeat(len(batch_paths), 1)
                ...
                decoded_texts = llava_tokenizer.batch_decode(output_ids[:, prompt_len:], skip_special_tokens=True)

                # 5. 保存结果
                for item_id, path, text in zip(batch_item_ids, batch_paths, decoded_texts):
                    # 内部处理仍使用 item_id 作为 key
                    generated_texts[item_id] = text.strip()
                    # 准备用于保存JSON的字典，使用文件名作为 key
                    filename = os.path.basename(path)
                    generated_texts_for_json[filename] = text.strip()
        ...
        # ★ 新增代码：保存生成的文本描述 ★
        # ================================================================= #
        print("\nSaving generated text descriptions to a JSON file...")
        try:
            # 使用图片文件名作为 key 保存
            with open(text_output_filename, 'w', encoding='utf-8') as f:
                json.dump(generated_texts_for_json, f, indent=4, ensure_ascii=False)
            print(f"Text descriptions successfully saved to {text_output_filename}")
        except Exception as e:
            print(f"Error saving text descriptions to file: {e}")
        # ================================================================= #
    ...
    # The output dimension is now the dimension of the SBERT model
    output_dim = sbert_dim

    # --- 6. 阶段二：批量编码所有文本为特征向量 ---
    print("\nStage 2: Batch encoding texts with Sentence-BERT...")
    if not generated_texts:
        print("No texts were generated, skipping encoding.")
        all_text_embeddings = torch.empty(0, sbert_dim).to(device)
    else:
        item_ids_encoded = list(generated_texts.keys())
        texts_to_encode = list(generated_texts.values())
        
        all_text_embeddings = sbert_model.encode(
            texts_to_encode, 
            batch_size=batch_size * 2, # SBERT可以处理更大的批次
            show_progress_bar=True,
            convert_to_tensor=True
        )

    # --- 7. 阶段三：准备最终特征 ---
    print("Stage 3: Preparing final features...")
    final_features = all_text_embeddings.cpu().numpy()
    feats = {item_id: feature for item_id, feature in zip(item_ids_encoded, final_features)}

    # --- 8. 处理缺失值并保存结果 ---
    total_items = len(item2id)
    missing_item_count = total_items - len(feats)
    print(f"Total items: {total_items}, Successfully processed: {len(feats)}, Missing/Failed: {missing_item_count}")
    
    avg_feature = [0.0] * output_dim
    if feats:
        avg_feature = np.array(list(feats.values())).mean(axis=0).tolist()
    else:
        print("Warning: No valid features extracted. Using a zero vector for all items.")
    
    ret = [feats.get(i, avg_feature) for i in range(total_items)]

    assert len(ret) == total_items, "Final feature count does not match item count."
    
    feature_array = np.array(ret, dtype=np.float32)
    print(f'Final feature shape: {feature_array.shape}')
    
    output_filename = os.path.join(target_path, 'image_feat.npy')
    np.save(output_filename, feature_array)
    print(f'Image features saved to {output_filename}')
