...
def extract_image_features(target_path, dataset_name='Games', model_name='all-MiniLM-L6-v2'):
    """
    Generates image features by extracting text features and then projecting them to 4096 dimensions.
    """
    print("Generating 'image' features via text projection...")
    
    # 1. Load metadata (same as text feature extraction)
    meta_file = os.path.join(target_path, f'meta-{dataset_name}.csv')
    df = pd.read_csv(meta_file)
    df.sort_values(by=['itemID'], inplace=True)
    
    print(f'Processing text for {df.shape[0]} items to generate image features.')
    ...
    # 2. Encode sentences to get base embeddings
    print("Encoding text with SentenceTransformer...")
    model = SentenceTransformer(model_name)
    sentence_embeddings = model.encode(sentences)
    print('Text encoding completed!')
    
    # 3. Project features from base dimension to 4096
    n_items, input_dim = sentence_embeddings.shape
    output_dim = 4096
    print(f"Projecting features from {input_dim} to {output_dim} dimensions...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    projection_layer = torch.nn.Linear(input_dim, output_dim).to(device)
    torch.nn.init.xavier_uniform_(projection_layer.weight)
    
    features_tensor = torch.from_numpy(sentence_embeddings).to(device)
    
    with torch.no_grad():
        projected_features = projection_layer(features_tensor).cpu().numpy()

    print(f"Projection complete. New feature shape: {projected_features.shape}")
    assert projected_features.shape == (n_items, output_dim)

    # 4. Save the new features as 'image_feat.npy'
    image_feat_output_file = os.path.join(target_path, 'image_feat.npy')
    np.save(image_feat_output_file, projected_features)
    
    # Create an empty file for missed items for consistency
    missed_items_file = os.path.join(target_path, "missed_img_itemIDs.csv")
    np.savetxt(missed_items_file, [], delimiter=",", fmt='%d')
    
    print(f"Projected image features saved to {image_feat_output_file}")
...
def main():
    ...
    if args.step in ['all', 'text', 'image']:
        ...
        if args.step in ['all', 'image']:
            ...
            # Corrected the function call to match the new definition
