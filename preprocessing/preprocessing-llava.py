...
def extract_image_features(source_path, target_path, dataset_name='Games', batch_size=8):
    ...
    # --- 3. 定义投影层 ---
    output_dim = 4096
    projection_layer = torch.nn.Linear(sbert_dim, output_dim).to(device)
    ...
    # --- 7. 阶段三：批量进行最终的维度投影 ---
    print("Stage 3: Projecting features...")
    with torch.no_grad():
        final_features = projection_layer(all_text_embeddings).cpu().numpy()
