...
def extract_image_features(target_path, dataset_name='Games'):
    """Extract image features using ViT model from image files"""
    ...
    # Add projection layer to get 4096 dimensions
    projection_layer = torch.nn.Linear(768, 4096).to(device)  # ViT-base outputs 768 dims
    ...
    print(f"Processing {len(item2id)} items...")
    
    # Process each item
    for asin, item_id in tqdm(item2id.items(), desc="Extracting image features"):
        ...
        if os.path.exists(image_path):
            try:
                ...
                with torch.no_grad():
                    ...
                    # Use the [CLS] token embedding as image feature (768 dims)
                    vit_feature = outputs.last_hidden_state[:, 0, :]  # [1, 768]
                    # Project to 4096 dimensions
                    # image_feature = projection_layer(vit_feature).cpu().numpy().flatten()  # [4096]
                    image_feature = vit_feature.cpu().numpy().flatten()
                    feats[item_id] = image_feature.tolist()
                    valid_features.append(image_feature)
                    
            except Exception as e:
                print(f"Error processing image {asin}.jpg: {e}")
                missing_images.append(item_id)
        else:
            missing_images.append(item_id)
    
    print(f"Successfully processed {len(feats)} images")
    print(f"Missing images: {len(missing_images)}")
    
    # Calculate average feature for missing images
    if valid_features:
        avg_feature = np.array(valid_features).mean(0).tolist()
    else:
        # If no valid features, create zero vector
        avg_feature = [0.0] * 4096  # 4096 dimensions
        print("Warning: No valid image features found, using zero vector as average")
    ...
    for i in range(len(item2id)):
        if i in feats:
            ...
            ret.append(avg_feature)
    
    print(f'Items missing image features: {len(missing_images)}')
    assert len(ret) == len(item2id)
    
    # Save features to target path
    feature_array = np.array(ret)
    print(f'Final feature shape: {feature_array.shape}')  # Should be [n_items, 4096]
    np.save(os.path.join(target_path, 'image_feat.npy'), feature_array)
    np.savetxt(os.path.join(target_path, "missed_img_itemIDs.csv"), missing_images, delimiter=",", fmt='%d')
    print(f'Image features saved! Feature dimension: {feature_array.shape}')
...
def main():
    ...
    if args.step in ['all', 'text', 'image']:
        ...
        if args.step in ['all', 'image']:
            ...
            image_features = extract_image_features(target_path, dataset_name)
