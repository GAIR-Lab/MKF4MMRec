...
class Config(object):
    ...
    def _load_dataset_model_config(self, config_dict, mg):
        ...
        # Use the directory where this script is located instead of cwd
        script_dir = os.path.dirname(os.path.abspath(__file__))
        src_dir = os.path.dirname(script_dir)  # Go up one level from utils to src
        cur_dir = os.path.join(src_dir, 'configs')
        file_list.append(os.path.join(cur_dir, "overall.yaml"))
        file_list.append(os.path.join(cur_dir, "dataset", "{}.yaml".format(config_dict['dataset'].split('-')[0])))
